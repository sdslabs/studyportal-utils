...
def addFileInDB(file):
    try:
        ...
        print(
          "Failed to insert record", file['title'], file['courseid'], error
        )
    ...
    for folder in departments[depts][course]:
        if course != 'id':
            if folder != 'id':
                ...
                if folder is not None and '_review' in folder:
                    continue
                ...
                if(len(filesInDriveFolder) != 0):
                    for item in filesInDriveFolder:
                        ...
                        if file_data['courseid'] == 0:
                            print(
                              file_data['title'],
                              "skipped, as course does not exist in database."
                            )
                            continue
                        syncAction(file_data)


def fileCheck(service):
    directoryStructure = open(STRUCTURE)
    ...
    for depts in departments:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            for course in departments[depts]:
                executor.submit(
                  courseScan, course, departments, depts, service
                )


def sync_drive(service):
    try:
        global connection
